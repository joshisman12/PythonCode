#this code currently cannot calculate the offset when the data is missing before the 
#pre-set endtime 
...
print(f'Postseismic endtime: {endtime1}  ')
for k in edat :
    inp = open(k)
    lines = inp.readlines()
    ...
    for i, line in enumerate(lines) :
        ...
        #    print(postend_pos4)
    cos_post_offset1 = postend_pos1 - cos_pos + cos_offset
    ...
    #eout.write(f'{k}  {post_offset1:8.2f}  {post_offset2:8.2f}  {post_offset3:8.2f} {post_offset4:8.2f}\n')
    eout.write(f'{k}  {cos_post_offset1:8.2f}\n')
    inp.close()
eout.close()
...
for k in ndat :
  ...
  #{post_offset4:8.2f}\n')
    nout.write(f'{k}  {cos_post_offset1:8.2f}\n')
    inp.close()
nout.close()
...
for k in udat :
    ...
    for i, line in enumerate(lines) :
        ...
        #    postend_pos4 = float(pos)
        #    print(postend_pos4)
    cos_post_offset1 = float(postend_pos1) - float(cos_pos) + float(cos_offset)
  ...
  #{post_offset4:8.2f}\n')
    uout.write(f'{k}  {cos_post_offset1:8.2f}\n')
    inp.close()
uout.close()    
